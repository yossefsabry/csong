...
char *lyrics_lrclib_fetch(const char *artist, const char *title, char *err, size_t err_cap) {
  ...
  size_t url_len = strlen(artist_enc) + strlen(title_enc) + 64;
  ...
  char *lyrics = json_get_string(buf.data, "plainLyrics", err, err_cap);
  if (lyrics && lyrics[0] == '\0') {
    free(lyrics);
    lyrics = NULL;
  }
  if (!lyrics) {
    lyrics = json_get_string(buf.data, "syncedLyrics", err, err_cap);
  }
  free(buf.data);
  if (!lyrics) {
    set_err(err, err_cap, "Lyrics not found");
